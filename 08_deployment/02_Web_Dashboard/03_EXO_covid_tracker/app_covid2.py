...
# streamlit run --server.port 4000 --server.address "0.0.0.0" app_covid2.py
...
def load_data(nrows=k_nrows):
    ...
    data_in["date"] = data_in["dateRep"].apply(
        lambda x: pd.to_datetime(x, format="%d/%m/%Y")
    )
    # data_in.drop(columns=["dateRep", "day", "month", "year"], inplace=True)
    data_in = data_in[
        ["date", "cases", "deaths", "countriesAndTerritories", "countryterritoryCode"]
    ]
...
## Config
st.set_page_config(page_title="my_covid_tracker", page_icon="", layout="wide")
...
fig.update_layout(
    yaxis_title="Cases", xaxis_title="Date", title="Cumulative cases over time"
)
...
country = st.selectbox(
    "Select a country", dataset["countriesAndTerritories"].sort_values().unique()
)
...
if country:
    with col1:
        ...
        fig.update_layout(
            yaxis_title="Cases", xaxis_title="Date", title="Cases over time"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig.update_layout(
            yaxis_title="Deaths", xaxis_title="Date", title="Deaths over time"
        )
        st.plotly_chart(fig, use_container_width=True)
